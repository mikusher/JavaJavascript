...
public class NeuralNet {
     ...
     *            Array containing the number of Neurons in
     *            each of the Hidden Layers
     * @param hiddenAcFnc
     *            Array containing the activation function of each
     *            Hidden Layer
     * @param outputAcFnc
        ...
        if (numberOfHiddenLayers == hiddenAcFnc.length) {
            ...
            for (int i = 0; i < numberOfHiddenLayers; i++) {
                if (i == 0) {
                    ...
                    try {
                        hiddenLayer.set(i,
                                        new HiddenLayer(numberofhiddenneurons[i],
                                                        hiddenAcFnc[i],
                                                        hiddenLayer.get(i - 1)
                    ...
                    hiddenLayer.get(i - 1)
                               .setNextLayer(hiddenLayer.get(i));
                }
            }
            if (numberOfHiddenLayers > 0) {
     ...
     * 
     * @param inputs
     *            Array of real values to be fed into the neural inputs
     */
     ...
     * 
     * @param inputs
     *            vector of values to be fed into the neural inputs
     */
    ...
    public void calc() {
        ...
        if (numberOfHiddenLayers > 0) {
            for (int i = 0; i < numberOfHiddenLayers; i++) {
                ...
                hl.setInputs(hl.getPreviousLayer()
                               .getOutputs());
                hl.calc();
            }
        }
        outputLayer.setInputs(outputLayer.getPreviousLayer()
                                         .getOutputs());
        outputLayer.calc();
